...
State GetState(PtPayload hstate, vec3 rayDir)
{
    State state;
    ...
    vec3 wpos0 = vec3(hstate.objectToWorld * vec4(pos0, 1.0));
    vec3 wpos1 = vec3(hstate.objectToWorld * vec4(pos1, 1.0));
    vec3 wpos2 = vec3(hstate.objectToWorld * vec4(pos2, 1.0));
    ...
    state.normal = (dot(world_normal, wgeom_normal) > 0.0) ? world_normal : -world_normal;
    state.ffnormal = dot(state.normal, rayDir) <= 0.0 ? state.normal : -state.normal;
    state.texCoord = texcoord0;
